import React, { useEffect, useState } from "react"; // side effects nad state
import { Link } from "react-router-dom"; // for navigating
import api from "../../services/api"; // axios instance
import NoteItems from "./NoteItems"; // Note Card
...
const AllNotes = () => {
  ...
  const fetchNotes = async () => {
    // backend call for notes fetching
    setLoading(true);
    try {
      ...
      setNotes(parsedNotes); // Array of Parsed Content Notes
    } catch (error) {
      setError(error.response.data.message); // setting error to error message we got, but we wont return error from here
      console.error("Error fetching notes", error); // only logging the error
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    //calling the function inside the useEffect here to fetch all notes
    fetchNotes();
  }, []); // done so dealing with the sid =e effects, so its called only once

  //to show an errors
  if (error) {
    // this is throwing the error
    // if error came it will be truthy value and wont be false which is default state
